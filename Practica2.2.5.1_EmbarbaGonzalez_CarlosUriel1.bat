...
for /f "tokens=1,* delims=;" %%a in (usuarios.txt) do (
    if "%%a"=="%usuario%" (
        if "%%b"=="%password%" (
            ...
            if %incorrectos% GTR %intentos_maximos% goto EliminarUsuario2
            timeout /nobreak /t 1 >nul
            goto MenuPrincipal
        )
    )
...
move /y usuarios_temp.txt usuarios.txt >nul
echo Has excedido el numero de intentos.
REM Registrar el evento en el log
echo [!datetime!] Usuario bloqueado por exceso de intentos fallidos: !usuario! >> %logfile%
timeout /nobreak /t 1 >nul
goto MenuPrincipal
